...
def perform_search(query: str, num_results: int = 5) -> str:
    ...
    if "latest" in query_lower or "news" in query_lower or "recent" in query_lower:
        time_limit = "w" # past week
    if "today" in query_lower or "24 hours" in query_lower:
        time_limit = "d" # past day

    # 1. Try SerpAPI if Key Exists
    if serp_api_key and GoogleSearch:
        try:
            print(f"Searching via SerpAPI: {query} (Time Limit: {time_limit})")
            params = {
                "engine": "google",
                "q": query,
    ...
    if DDGS:
        try:
            print(f"Searching via DuckDuckGo: {query} (Time Limit: {time_limit})")
            with DDGS() as ddgs:
                # timelimit: 'd' (day), 'w' (week), 'm' (month), 'y' (year)
                results = list(ddgs.text(query, max_results=num_results, timelimit=time_limit))
